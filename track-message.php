...
class TrackMessage{
    ...
    public function policyUrlFieldCallback(){
        ...
        $html .= sprintf('type="text">%s</textarea>', esc_html__($this->cookie_policy_url, 'track-message'));
        $html .= sprintf('<p class="%s">%s<p>', esc_attr($class), esc_html($text));
    ...
    public function policyLinkFieldCallback() {
        ...
        $html.= sprintf('type="text">%s</textarea>', esc_html__($this->cookie_policy_link, 'track-message'));
        $html .= sprintf('<p class="%s">%s<p>', esc_attr($class), esc_html($text));
    ...
    public function selectPolicyLinkFieldCallback(){
        ...
        $name = ('tmssg_content_options[select_policy_link]');      
        $html = sprintf('<input type="%s" name="%s" %s value="%s">
        ',esc_attr($type), esc_attr($name), esc_attr($checked), esc_attr($value));
        $html .= sprintf('<p class="%s">%s<p>', esc_attr($class), esc_html($text));
        echo $html;
    ...
    public function policyTabCallback(){
        ...
        {
            $html .= sprintf('<option value="%s"'.selected(esc_attr($this->content_options['policy_tab_selector']), esc_attr($key), false).'>%s</option>', esc_attr($key), esc_html($value));
        }
        $html .= ('</select>');
        $html .= sprintf('<p class="%s">%s<p>', esc_attr($class), esc_html($text));
        echo $html;    
    ...
    public function tmssgShowMessage(){
        ...
        $id_url = ('TrackMessageCookieNotification__url-Id--2443');
